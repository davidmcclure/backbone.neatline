...
describe('Neatline.Collection', function() {
  ...
  describe('update', function() {
    ...
    it('should issue GET request with passed parameters', function() {

      // ------------------------------------------------------------------
      // `update` should call `fetch` with GET parameters derived from the
      // passed `params` object and invoke the passed callback on success.
      // ------------------------------------------------------------------
      ...
      // Update.
      coll.update({ key: 'val'}, callback);

      // Capture outoing GET request.
      var request = _t.getLastRequest();
      _t.respondLast200(_t.json.collection);
      waitsFor(function() { return done; });

      // Check method and route.
      expect(request.method).toEqual('GET');
      expect(request.url).toEqual('api?key=val');
